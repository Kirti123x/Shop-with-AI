...
import { Search, Ruler, Eye, EyeOff, User, X, Info, Maximize2, Minimize2 } from 'lucide-react'
 ...
 * both fed from StyleHub's own size charts (see sizeCharts.js). Can be
 * expanded to a fullscreen view, out of the small chat window. */
export default function ChatCompare({ product }) {
  ...
  if (!product) {
    inner = (
      <p className="text-xs text-myntra-gray bg-myntra-bg rounded-xl p-3">
        Open a product first, then tap "Visualise &amp; Compare" again to preview it on your shape.
      </p>
    )
  } else if (!CATEGORY_REGION[product.category]) {
    inner = (
      <p className="text-xs text-myntra-gray bg-myntra-bg rounded-xl p-3">
    ...
    inner = (
      <div className={expanded ? 'space-y-3 max-w-xl mx-auto' : 'space-y-2.5'}>
          ...
          <div className="bg-white rounded-xl p-3 shadow-card">
            ...
            <div className="flex justify-center bg-myntra-bg rounded-lg py-2">
              <svg
                ...
                className={expanded ? 'h-[420px]' : 'h-56'}
  ...
  if (expanded) {
    return (
      <div className="fixed inset-0 z-[100] bg-myntra-bg flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-white shrink-0">
          <p className="font-semibold text-sm text-myntra-dark flex items-center gap-1.5">
            <Ruler size={15} className="text-myntra-pink" /> Visualise &amp; Compare
          </p>
          <button
            onClick={() => setExpanded(false)}
            title="Exit fullscreen"
            className="text-myntra-gray hover:text-myntra-dark p-1"
          >
            <Minimize2 size={18} />
          </button>
        </div>
  ...
  return (
    <div className="mt-2">
      {product && CATEGORY_REGION[product.category] && bodySource && (
        <div className="flex justify-end mb-1">
          <button
            onClick={() => setExpanded(true)}
            title="Take out of chat window"
            className="flex items-center gap-1 text-[10px] font-medium text-myntra-gray hover:text-myntra-dark bg-white border border-gray-200 rounded-full px-2 py-1"
          >
